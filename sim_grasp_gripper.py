...
def run(database_path, start_idx, objs_num):
    cid = p.connect(p.GUI)  
    gripper = gripper_sim.GripperSimAuto(p, [1, 0, 0.5])  
    env = SimEnv(p, database_path, gripper.gripperId) 
    camera = Camera()  
    ggcnn = GGCNNNet('ggcnn_new_dataset/output/models/221018_1353_/epoch_0909_acc_0.7102.pth', device="cpu")    

...
GRASP_MAX_W = 0.08     # maximum open width


def clacPlane(pt1, pt2, pt3):
    """
    according to ax+by+cz+d=0
...
class Mesh:
    """
    mesh class, read .obj files and transform the coordinate to generate depth images
    """
    ...
    def renderTableImg(self, mask_id, size=(0.8, 0.8), unit=0.001):
        ...
        for face in self.faces:
            ...
            # get pixel coordinates inside the triangle (x,y)
            pts_pixel = ptsInTriangle(pt1_pixel, pt2_pixel, pt3_pixel)

            if len(pts_pixel) == 0:
                continue

            # convert to real coordinates m
            ...
            #  convert pts_pixel to image coordinates

...
def run(net):
    ...
    # train data
    train_data = Cornell("./ggcnn/cornell",output_size=300,random_rotate=True,random_zoom=True)

    train_dataset = torch.utils.data.DataLoader(train_data,batch_size = batch_size,shuffle = True)
    
    # validation data
    val_data = Cornell("./ggcnn/cornell",output_size=300,random_rotate=True,random_zoom=True)
...
if __name__ == '__main__':
    net = GGCNN(4)
    run(net)
    torch.save(net,'ggcnn/model/model_optimized.pt')